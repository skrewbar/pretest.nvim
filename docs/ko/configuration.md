...
> `<C-r>`과 `<C-S-r>`를 구분하지 못하는 터미널 에뮬레이터를 사용하는 경우 다른 키로 변경해 주세요.

## `languages`

`languages`는 Neovim **filetype**(`:set filetype?`의 값, 예: `cpp`, `python`, `rust`)을 빌드/실행 방법에 대응시킵니다:
...
- `exec`는 셸을 거치지 않고 직접 실행됩니다. 따옴표 처리, 글로빙, `~`, `&&`는 동작하지 않습니다.
- `exec`와 `args`의 각 원소에서, 값이 `$src`이면 소스의 절대 경로로, `$bin`이면 바이너리 경로로 확장됩니다. 문자열 안에 포함된 경우(예: `"--out=$bin"`)는 확장되지 **않습니다**. 이러한 기능이 필요한 경우에는 함수를 사용하세요.
- 함수는 `ctx = { src_path = "...", bin_path = "..." }`를 받아 문자열(`exec`) 또는 리스트(`args`)를 반환합니다. 반환값은 그대로 사용됩니다.
- `bin_path` 혹은 `$bin`은 제안일 뿐이므로 다른 위치를 사용해도 됩니다.
...
C++의 컴파일러와 플래그만 바꾸기 (나머지 항목은 유지됩니다):
...
Java. 클래스 이름이 파일 이름에서 오고 클래스 파일이 디렉터리에 생성되기 때문에 함수를 사용합니다:
